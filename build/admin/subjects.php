...
if(isset($_POST['submit'])) 
{
  ...
  // if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["thumbnail"]["tmp_name"], $target_file)) {
      mysqli_query($conn, "INSERT INTO courses (`name`, `category`, `duration`,`image`, `date`) VALUES ('$name', '$category', '$duration', '$image', '$today')") or die(mysqli_error($conn));
        ...
        <div class="card">
         ...
         <div class="form-group">
          <label for="name">Course Name</label>
            <input type="text" name="name" placeholder="Course name" requried class="form-control">
         </div>
         <div class="form-group">
          <label for="name">Course Category</label>
           <select name="category" id="category" class="form-control">
            <option value="">Select category</option>
            <option value="web-design-and-developement">Web Design & Developement</option>
              ...
              <tbody>
               ...
               $curse_query= mysqli_query($conn,'SELECT * FROM Courses');
                while($course = mysqli_fetch_object($curse_query)) { ?>
                  <tr>
                    <td><?=$count++?></td>
                    <td><img src="../dist/uploads/<?=$course->image?>" height="100" alt="<?=$course->title?>" class="border"></td>
                    <td><?=$course->name?></td>
